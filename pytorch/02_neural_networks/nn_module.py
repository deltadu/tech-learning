...
It provides a clean way to organize parameters, sub-modules, and forward logic.
"""
...
adaptive_pool = nn.AdaptiveAvgPool2d((1, 1))  # output size (1, 1) regardless of input
...
class FunctionalNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=0.5, training=self.training)  # Note: training flag
        x = self.fc2(x)

...
Get started with beginner-friendly examples. All run on a classical simulator — **no quantum hardware needed!**

```bash
pip install qiskit qiskit-aer
python intro_examples.py        # Run all 8 examples
python intro_examples.py 3      # Run specific example (e.g., Bell state)
```

See [`intro_examples.py`](./intro_examples.py) for complete runnable code covering:
...
| 4 | Inspect quantum state without measurement |
| 5 | Apply different gates (X, H, rotations) |
| 6 | Measure in different bases (Z vs X) |
| 7 | Grover's search algorithm |
| 8 | Multi-qubit GHZ state |
| ref | Common gates quick reference |
